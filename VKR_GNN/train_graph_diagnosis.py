...
from utils.train_graph_utils import train_acc_to_plot
from utils.train_graph_utils import val_acc_to_plot
...
def parse_args():
    ...
    parser.add_argument('--data_name', type=str, default='XJTUGearboxRadius', help='the name of the data')
    parser.add_argument('--Input_type', choices=['TD', 'FD', 'other'], type=str, default='FD', help='the input type decides the length of input')
    parser.add_argument('--data_dir', type=str, default= "D:\project\Project_Python\ITMO_course\VKR_GNN\data\XJTU_Gearbox", help='the directory of the data')
...
if __name__ == '__main__':
    ...
    if args.task == 'Node':
        sub_dir = args.task + '_' + args.model_name+'_'+args.data_name + '_' + args.Input_type + '_' +\
                  datetime.strftime(datetime.now(), '%m%d-%H%M%S')
    else:
        sub_dir = args.task + '_' + args.model_name + '_' + args.pooltype + '_' + args.data_name + '_' + \
                  args.Input_type + '_' + datetime.strftime(datetime.now(), '%m%d-%H%M%S')
    ...
    with open('MLP_F.data', 'wb') as filehandle:
        pickle.dump(train_acc_to_plot, filehandle)
        pickle.dump(val_acc_to_plot, filehandle)
